...
bool SCAPI SCCC SteamController_TurnOff(const SteamControllerDevice *pDevice) {
  if (!pDevice)
    return false;
...
void SCAPI SCCC SteamController_SaveMelodies(const SteamControllerDevice *pDevice, uint8_t startupMelody, uint8_t shutdownMelody) {
  SteamController_HIDFeatureReport featureReport;
